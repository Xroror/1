...
var myGameArea = {
	...
	start: function(wid, hei){
		...
		this.update = setInterval(GameUpdate, 20);
...
function GameUpdate(){ //called every 20ms //checks for user input
	...
	if (myGameArea.key && myGameArea.key == 65 || myGameArea.key && myGameArea.key == 37) {
		if(inputdelay[inputdelay.length-1] != 1 && inputdelay[inputdelay.length-1] !=3){
			inputdelay[inputdelay.length] = 3;
		}
	}
	if (myGameArea.key && myGameArea.key == 68 || myGameArea.key && myGameArea.key == 39) {
		if(inputdelay[inputdelay.length-1] != 3 && inputdelay[inputdelay.length-1] !=1){
			inputdelay[inputdelay.length] = 1;
		}
	}
	if (myGameArea.key && myGameArea.key == 87 || myGameArea.key && myGameArea.key == 38) {
		if(inputdelay[inputdelay.length-1] != 4 && inputdelay[inputdelay.length-1] !=2){
			inputdelay[inputdelay.length] = 2;
		}
	}
	if (myGameArea.key && myGameArea.key == 83 || myGameArea.key && myGameArea.key == 40) {
		if(inputdelay[inputdelay.length-1] != 2 && inputdelay[inputdelay.length-1] !=4){
			inputdelay[inputdelay.length] = 4;
		}
	}
...
function sendscore(){ //sends the score towards php script
	...
	xhttp.open("POST", "/games/snake/dev/snakescore.php", true);
